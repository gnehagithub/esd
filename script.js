...
table_data += '&nbsp;&nbsp;';
table_data += `<button class="btn btn-danger" onclick=deleteData('${records[i]._id}','${records[i].name}')>Delete</button>`;
table_data += `</td>`;
table_data += `</tr>`;
}
...
user_input = confirm(`Are you sure you want to delete ${name}'s record?`);
if(user_input) {
fetch(api_url, {
method: "DELETE",
headers: {
